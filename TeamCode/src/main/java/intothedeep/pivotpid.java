...
//        int slidepos = (rightslidepos + leftslidepos)/2;
        double encoderposition = encoder.getVoltage() / 3.2 * 360;
        double pid = controller.calculate(encoderposition, targetangle);
        double ff = Math.cos(Math.toRadians(targetangle /ticks_in_degree)) * f;
